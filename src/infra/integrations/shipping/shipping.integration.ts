...
import { ShippingAdapter, IShippingCalculateInput, IShippingCalculateOutput } from '../../../domain/adapters';

@Injectable()
export class ShippingIntegration extends ShippingAdapter {
  private readonly baseUrl = process.env.SHIPPING_API_URL || 'https://api.melhorenvio.com.br';
  private readonly apiKey = process.env.SHIPPING_API_KEY;

  async calculate(input: IShippingCalculateInput): Promise<IShippingCalculateOutput> {
    ...
    const cheapestOption = Array.isArray(data) ? data[0] : data;
